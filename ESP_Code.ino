...
String serverNameWithAuth, st, content, outputsState;
// Your IP address or domain name with URL path
String serverName = "http://iot.electrocse.com/esp-outputs-action.php?action=outputs_state&board=";
...
void setup()
{
  ...
  Serial.println("Reading EEPROM Auth Email");
  String theAuthMail;
  for (int i = 64; i < 96; ++i)
  {
    theAuthMail += char(EEPROM.read(i));
  }
  Serial.println();
  Serial.print("Auth Email: ");
  Serial.println(theAuthMail);
  ...
  if (testWifi())
  {
    ...
    Serial.print("Auth Email: ");
    Serial.println(theAuthMail);    
    serverNameWithAuth = serverName + boardNo + "&authMail=" + theAuthMail;
...
{
  {
    server.on("/", []() {
      ...
      content += " </p>\n"
                 ...
                 "\t\t\t<label><b>Email </b> [ Use same email at www.iot.electrocse.com ]</label> \n"
                 "            <input type=\"email\" placeholder=\"Enter Your Email\" name=\"authEmail\" length = 32 required> \n"
                 "\t\t\t<label><b>Board Number </b> [ If this is your first ESP board then enter 1 ]</label> \n"
                 "\t\t\t<input type=\"number\" placeholder=\"Enter ESP Board Number\" name=\"board\" length = 10 required>           \t\t\n"
    ...
    server.on("/setting", []() {
      ...
      String qAuthEmail = server.arg("authEmail");
      String qboard = server.arg("board");

      if (qsid.length() > 0 && qpass.length() > 0 && qAuthEmail.length() > 0 && qboard.length() > 0) {
        ...
        Serial.println("Writing eeprom Auth Email: ");
        for (int i = 0; i < qAuthEmail.length(); ++i)
        {
          EEPROM.write(64 + i, qAuthEmail[i]);
          Serial.print("Wrote: ");
          Serial.println(qAuthEmail[i]);
        }

        Serial.println("Writing eeprom Board No.: ");
        for (int i = 0; i < qboard.length(); ++i)
        {
          EEPROM.write(96 + i, qboard[i]);
          Serial.print("Wrote: ");
